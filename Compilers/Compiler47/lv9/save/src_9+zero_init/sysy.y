...
%token INT RETURN VOID CONST LE GE EQ NE AND OR IF ELSE WHILE BREAK CONTINUE FOR
%token <str_val> IDENT 
...
ClosedStmt
    ...
    | FOR '(' LVal '=' Exp ';' Exp ';' LVal '=' Exp ')' ClosedStmt {
        auto left = new SingleStmtAST();
        left->LVal = *unique_ptr<string>($3);
        left->Exp = unique_ptr<BaseAST>($5);
        left->type = "assignment";
        auto right = new SingleStmtAST();
        right->LVal = *unique_ptr<string>($9);
        right->Exp = unique_ptr<BaseAST>($11);
        right->type = "assignment";
        auto stmt = new StmtAST();
        stmt->type = "for";
        stmt->forStmt1 = unique_ptr<BaseAST>(left);
        stmt->Exp = unique_ptr<BaseAST>($7);
        stmt->forStmt2 = unique_ptr<BaseAST>(right);
        stmt->forStmt = unique_ptr<BaseAST>($13);
        $$ = stmt;
    }
    ;
OpenStmt
    ...
    | FOR '(' LVal '=' Exp ';' Exp ';' LVal '=' Exp ')' OpenStmt {
        auto left = new SingleStmtAST();
        left->LVal = *unique_ptr<string>($3);
        left->Exp = unique_ptr<BaseAST>($5);
        left->type = "assignment";
