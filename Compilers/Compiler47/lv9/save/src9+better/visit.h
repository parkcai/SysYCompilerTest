...
void init_zero()
{
    if(allow_out&&zero_init>0)
    {
        ...
        tempstr += ".zero " + to_string(zero_init * 4) + "\n";
        zero_init = 0;
    }
}
void copy()
...
void visit(koopa_raw_basic_block_t bb, bool with_addi)
{
    if(allow_out)
    {
        instruction temp;
        temp.instruct = "str";
        temp.str = string(bb->name + 1) + ":\n";
        instructions.push_back(temp);
        if(with_addi)
        {
            instruction temp;
            temp.instruct = "addsp";
            temp.imm = -stack_size.top();
            instructions.push_back(temp);
        }
    }
    if(allow_out)out << bb->name + 1 << ":\n" << flush;
    //access sp
    if (with_addi&&allow_out)
...
int visit(const koopa_raw_load_t &load)
{
    ...
    if (src->kind.tag == KOOPA_RVT_GLOBAL_ALLOC)
    {
        ...
        {
            instruction temp;
            temp.instruct = "load_global";
            temp.dest = new_pos;
            temp.label = global_values[src];
            instructions.push_back(temp);
        }
        lw(0, 0, 0);
        sw(0, new_pos);
...
void visit(const koopa_raw_branch_t &branch)
{
    ...
    {
        instruction temp;
        temp.instruct = "branch";
        temp.pos1 = pos;
        temp.str = true_label;
        temp.str2 = false_label;
        instructions.push_back(temp);
    }
    lw(0, pos);
...
string visit(const koopa_raw_global_alloc_t &global)
{
    ...
    case KOOPA_RVT_ZERO_INIT:
        ...
        if(allow_out)
        {
            instruction temp;
            temp.instruct = "str";
            temp.str = string("\t.zero ") + to_string(cal_size(global.init->ty)) + '\n';
            instructions.push_back(temp);
        }
        break;
    case KOOPA_RVT_INTEGER:
        if(allow_out)
        {
            ...
            if(allow_out)
            {
                instruction temp;
                temp.instruct = "str";
                temp.str = string("\t.word ") + to_string(value) + '\n';
                instructions.push_back(temp);
            }
        }
        break;
        ...
        if (value->kind.tag == KOOPA_RVT_INTEGER)
        {
            ...
            if(allow_out)
            {
                ...
                if(init_value)
                {
                    ...
                    tempstr += string("\t.word ") + to_string(init_value) + '\n';
                }
                else
                    ++zero_init;
            }
