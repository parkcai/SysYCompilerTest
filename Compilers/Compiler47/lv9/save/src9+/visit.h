...
#include <math.h>
using namespace std;
//using std::ofstream, std::map, std::cif(allow_out)out, std::cerr;

#define debug(x) cerr << __LINE__ << ' ' << x << '\n';
#define LINE if(allow_out)out << __LINE__ << std::endl;
#define C // cif(allow_out)out<<__LINE__<<endl;
extern ofstream out;
...
void visit(koopa_raw_function_t func)
{
    ...
    for (size_t i = 0; i < func->bbs.len; ++i)
    {
        ...
        // if(allow_out)out<<"bbs.buffer["<<i<<"] is ok\n";
    }
}
void visit(koopa_raw_basic_block_t bb, bool with_addi)
{
    if(allow_out)out << bb->name + 1 << ":\n"
        << flush;
    if (with_addi)
        addi(18, 18, -stack_size.top());//102400
    // if(allow_out)out << "\taddi sp, sp, -1024"<<"#!!!"<<endl;
    for (size_t i = 0; i < bb->insts.len; ++i)
    {
        visit((koopa_raw_value_t)bb->insts.buffer[i]);
        // if(allow_out)out<<"insts.buffer["<<i<<"] is ok\n";
    }
    // cif(allow_out)out<<__LINE__<<bb->insts.len<<'\n';
}
int visit(koopa_raw_value_t value)
{
    ...
    // cif(allow_out)out<<value->kind.tag<<endl;
    // cif(allow_out)out<<__LINE__<<endl;
    return pos;
}
void visit(const koopa_raw_return_t &ret)
{
    ...
    if (ret.value)
    {
        ...
        // if(allow_out)out << "\tlw a0, " << pos <<"(sp)\n"<<flush;
    }
    // if(allow_out)out<< "\taddi sp, sp, 1024"<<endl;
    addi(18, 18, stack_size.top());
...
int visit(const koopa_raw_load_t &load)
{
    ...
    if (src->kind.tag == KOOPA_RVT_GLOBAL_ALLOC)
    {
        ...
        if(allow_out)out << "\tla " << reg_names[0] << ", " << global_values[src] << endl;
        lw(0, 0, 0);
        sw(0, new_pos);
        return new_pos;
    }
...
void visit(const koopa_raw_store_t &store)
{
    ...
    // cif(allow_out)out<<pos<<' '<<cnt_pos<<endl;
    koopa_raw_value_t dest = store.dest;
    if (dest->kind.tag == KOOPA_RVT_GLOBAL_ALLOC)
    {
        assert(global_values.count(dest));
        if(allow_out)out << "\tla s11, " << global_values[dest] << std::endl;
        lw(0, pos);
        // if(allow_out)out<<"\tlw    "<<reg_names[0]<<", "<<pos<<"(sp)\n"<<flush;
        sw(0, 0, get_reg_num("s11"));
        // if(allow_out)out << "\tsw    " << reg_names[0] << ", 0(s11)" << std::endl;
        return;
    }
    ...
    // if(allow_out)out<<"\tlw    "<< reg_names[0] <<", "<<pos<<"(sp)\n"<<flush;
    sw(0, store_pos);
    // if(allow_out)out<<"\tsw    "<< reg_names[0] <<", "<<store_pos<<"(sp)\n"<<flush;
...
void visit(const koopa_raw_branch_t &branch)
{
    ...
    lw(0, pos);
    // if(allow_out)out<<"\tlw    "<< reg_names[0] <<", "<<pos<<"(sp)\n"<<flush;
    if(allow_out)out << "\tbnez  " << reg_names[0] << ", " << true_label
        << std::endl;
...
int visit(const koopa_raw_call_t &call)
{
    ...
    sw(17, ra_save);
    // if(allow_out)out <<"\tsw    ra" <<", "<<ra_save<<"(sp)\n"<<flush;
    ...
    for (size_t i = 0; i < call.args.len; i++)
    {
        if (i < 8)
        {
            save[i] = cnt_pos += 4;
            sw(7 + i, save[i]);
            // if(allow_out)out <<"\tsw    "<< reg_names[7+i] <<", "<<save[i]<<"(sp)\n"<<flush;
        }
        ...
        if (i < 8)
            lw(7 + i, pos);
        // if(allow_out)out<<"\tlw    "<< reg_names[7+i] <<", "<<pos<<"(sp)"<<endl;
        else
        {
            move(pos, 4 * i - 32);
        }
    ...
    int new_pos = cnt_pos += 4;
    sw(7, new_pos);
    //if(allow_out)out << "\tsw    a0" << ", " << new_pos << "(sp)\n"<< flush;
    for (int i = 0; i < min((int)call.args.len, 8); i++)
        lw(7 + i, save[i]);
    // if(allow_out)out <<"\tlw    "<< reg_names[7+i] <<", "<<save[i]<<"(sp)\n"<<flush;
    assert(reg_names[17] == "ra");
    lw(17, ra_save);
    // if(allow_out)out <<"\tlw    ra" <<", "<<ra_save<<"(sp)\n"<<flush;
...
int visit(const koopa_raw_func_arg_ref_t &ret)
{
    ...
    if (index < 8)
    {
        int pos = cnt_pos += 4;
        sw(7 + index, pos);
        // if(allow_out)out << "\tsw "<<reg_names[7+index]<<", "<<pos<<"(sp)\n"<<flush;
        return pos;
    }
    else
    {
...
string visit(const koopa_raw_global_alloc_t &global)
{
    ...
    case KOOPA_RVT_INTEGER:
        if(allow_out)out << "\t.word " << global.init->kind.data.integer.value << endl;
        break;
...
int visit(const koopa_raw_get_elem_ptr_t &get_elem_ptr)
{
    ...
    if (get_elem_ptr.src->kind.tag == KOOPA_RVT_GLOBAL_ALLOC)
    {
        ...
        if(allow_out)out << "\tla    " << reg_names[0] << ", " << global_values[get_elem_ptr.src] << endl;
    }
    ...
    else
    {
        ...
        lw(0,pos);
        // int pos = value_map[get_elem_ptr.src];
        // if(allow_out)out << "\t"<<get_elem_ptr.src->kind.tag << endl;
        // assert(0);
    }
    if(allow_out)out << "\tli    s11, " << elem_size << std::endl;
    if(allow_out)out << "\tmul   s11, s11, " << reg_names[1] << std::endl;
    if(allow_out)out << "\tadd   " << reg_names[0] << ", " << reg_names[0] << ", s11" << std::endl;
    int new_pos = cnt_pos += 4;
    reg2stack(0, new_pos);
...
int visit(const koopa_raw_get_ptr_t &get_ptr)
{
    ...
    li(0, elem_size);
    stack2reg(index_pos, 1);
    mul(0, 0, 1);
    stack2reg(pos, 1);
    add(0, 0, 1);
    reg2stack(0, cnt_pos += 4);
    return cnt_pos;
}