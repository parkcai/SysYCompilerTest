...
int RISCV::calc_func_size(koopa_raw_function_t kfunc, bool &has_call)
{
    int total_size = 0;
...
int RISCV::calc_blk_size(koopa_raw_basic_block_t kblk, bool &has_call)
{
...
int RISCV::calc_inst_size(koopa_raw_value_t kval)
{
...
int RISCV::calc_type_size(koopa_raw_type_t ty)
{
    switch(ty->tag)
    {
...
void RISCV::load_to_reg(koopa_raw_value_t kval, const char *reg)
{
...
void RISCV::store_to_stack(int addr, const char *reg)
{
    if(addr < -2048 || addr > 2047)
    {
        output << "\tli t6, " << addr << endl;
...
void RISCV::gen_riscv_value_aggregate(koopa_raw_value_t kval)
{
...
void RISCV::gen_riscv_value_global_alloc(koopa_raw_value_t kalloc)
{
...
void RISCV::gen_riscv_value_load(const koopa_raw_load_t *kload, int addr)
{
    output << endl;
...
void RISCV::gen_riscv_value_store(const koopa_raw_store_t *kstore)
{
    output << endl;
...
void RISCV::gen_riscv_value_get_ptr(const koopa_raw_get_ptr_t *kget, int addr)
{
    output << endl;
...
void RISCV::gen_riscv_value_get_elem_ptr(const koopa_raw_get_elem_ptr_t *kget, int addr)
{
    output << endl;
...
void RISCV::gen_riscv_value_binary(const koopa_raw_binary_t *kbinary, int addr)
{
    output << endl;
...
void RISCV::gen_riscv_value_branch(const koopa_raw_branch_t *kbranch)
{
    output << endl;
...
void RISCV::gen_riscv_value_jump(const koopa_raw_jump_t *kjump)
{
    output << endl;
    output << "\tj " << current_func_name << "_" << kjump->target->name + 1 << endl;
}

void RISCV::gen_riscv_value_call(const koopa_raw_call_t *kcall, int addr)
{
    output << endl;
...
void RISCV::gen_riscv_value_return(const koopa_raw_return_t *kret)
{
    output << endl;
