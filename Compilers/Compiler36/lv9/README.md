# Qin Dawei's Compiler Principle lab project
...
每个koopa slice有三个成员，分别是slice items的buffer，slice的length和kind。
...
在tag之后有一个名叫data的union，针对array，pointer和function三种tag进行不同处理，array存储base pointer和len，pointer存储base pointer，function存储params和return pointer。每个pointer都是一个koopa_raw_type_kind的struct，params是koopa_raw_slice_t的结构。
